setwd("~/Documents/Gabe/myLSUcloud/Research/HGRM/Analyses/personalization/data_2016_june")
fileH1i <- "H1individual.csv"
fileH1a <- "H1aggregate.csv"
fileCompl <- "BernaComplete_2011_20160630.csv"
fileRep <- "BernaRepeatedPair_2011_20160630_BothChannels.csv"
fileAll <- "BernaAllReservation_2011_20160630.csv"

individual <- read.csv(fileH1i)
individual$Date <- (gsub("/", "-", individual$Date))
...
lines(vp$Month+5, (vp$pcStays*100), type="h", lwd=10,
     lty=linetype[1], col=rgb(0,0,1,0.2)) 
lines(ph$Month-5, (ph$pcStays*100), type="h", lwd=10,
     lty=linetype[1], col=rgb(1,0,0))
dev.off()
...
# Plot adopters and percentage of stays 
...
## EXTRA ANALYSIS OF NUMBER OF CATEGORIES
H <- na.omit(x[, c("Service", "Count.of.Personalization.Categories.Used", "ROOM_TYPE", "LoS", "ADR", "Personalization", "ADULTS", "CHILDREN", "PrefPrice")])
H$Service <- ordered(H$Service, c(2.5, 5, 7.5, 10))
h0 <- ordinal::clm(Service ~ Count.of.Personalization.Categories.Used + ADR + LoS + ROOM_TYPE + ADULTS + CHILDREN + PrefPrice, data = subset(H, Personalization == 1 & ADULTS + CHILDREN > 0 & PrefPrice < 50), Hess = TRUE)
tabH <- round(cbind(coef(summary(h0)), exp(coef(h0))), digits = 3)
tabH[1:3, ncol(tabH)] <- NA
tabH

H <- na.omit(x[, c("Value", "Count.of.Personalization.Categories.Used", "ROOM_TYPE", "LoS", "ADR", "Personalization", "ADULTS", "CHILDREN", "PrefPrice")])
H$Value <- ordered(H$Value, c(2.5, 5, 7.5, 10))
h0 <- ordinal::clm(Value ~ Count.of.Personalization.Categories.Used + ADR + LoS + ROOM_TYPE + ADULTS + CHILDREN + PrefPrice, data = subset(H, Personalization == 1 & ADULTS + CHILDREN > 0 & PrefPrice < 50), Hess = TRUE)
tabH <- round(cbind(coef(summary(h0)), exp(coef(h0))), digits = 3)
tabH[1:3, ncol(tabH)] <- NA
tabH
##
...
mf$DEPARTUREDATE <- as.Date(mf$DEPARTUREDATE, format = "%Y-%m-%d")
mf$ARRIVALDATE <- as.Date(mf$ARRIVALDATE, format = "%Y-%m-%d")
mf$BOOKING_DATE <- as.Date(mf$BOOKING_DATE, format ="%Y-%m-%d")
...
##########################
#Extra analysis of Multiple Repeated Guests over time
#Percentage of repeat
repmult <- read.csv(fileAll)
...
#Proportions by number of repeats
...
#Personalization extent by channel
levs <- sapply(c(as.list(new_levels[1:3]), list(hp), list(internet_channels), list(gds_channels), list(expedia), list(cc)), function(g) {
